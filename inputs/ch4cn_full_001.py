...
ax[0,0].scatter(x, cm_ene_pred_traj, s=marker_size)
ax[0,0].scatter(x, cm_ene_traj_true, s=marker_size)
ax[0,0].set_title("Coulomb matrix")
ax[0,1].scatter(x, slatm_ene_pred_traj, s=marker_size, label="SLATM")
ax[0,1].scatter(x, cm_ene_traj_true, s=marker_size)
ax[0,1].set_title("SLATM")
ax[1,0].scatter(x, aslatm_ene_pred_traj, s=marker_size, label="aSLATM")
ax[1,0].scatter(x, cm_ene_traj_true, s=marker_size)
ax[1,0].set_title("aSLATM")
ax[1,1].scatter(x, acsf_ene_pred_traj, s=marker_size, label="Prediction")
ax[1,1].scatter(x, cm_ene_traj_true, s=marker_size, label="True")
ax[1,1].legend()
...
plt.tight_layout()
plt.savefig("../plots/ch4cn_full_001_traj.png", dpi=150)
# plt.show()

fog, ex = plt.subplots(2, 2, figsize=(12,8), sharex='col', sharey='row')
ex[0,0].scatter(cm_ene_true, cm_ene_pred, alpha=0.5, label="Coulomb matrix", s=marker_size)
ex[0,0].plot([min(cm_ene_true), max(cm_ene_true)], [min(cm_ene_true), max(cm_ene_true)], c="black")
ex[0,0].set_title("Coulomb matrix")
ex[0,1].scatter(slatm_ene_true, slatm_ene_pred, alpha=0.5, label="SLATM", s=marker_size)
ex[0,1].plot([min(slatm_ene_true), max(slatm_ene_true)], [min(slatm_ene_true), max(slatm_ene_true)], c="black")
ex[0,1].set_title("SLATM")
ex[1,0].scatter(aslatm_ene_true, aslatm_ene_pred, alpha=0.5, label="aSLATM", s=marker_size)
