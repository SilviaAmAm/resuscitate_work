...
np.savez("../outputs/ch4cn_ho_007.npz", idx_train, idx_test)
...
estimator = sklearn.pipeline.make_pipeline(
                qmlearn.representations.AtomCenteredSymmetryFunctions(data, nbasis=15, cutoff=4.1, precision=2),
                qmlearn.models.NeuralNetwork(hl3=0)
                )

# Saving the model and training on all the samples in the model
pickle.dump(estimator, open('../outputs/ch4cn_ho_007.pickle', 'wb'))
indices = np.arange(len(idx_train))
with open('../outputs/ch4cn_ho_007.csv', 'w') as f:
    for i in indices:
        f.write('%s\n' % i)
