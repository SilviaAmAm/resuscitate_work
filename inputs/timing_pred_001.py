...
model_paths = ["../tmp_models/hc%i_sq"%i for i in range(1,7)]
models_rep_time = []
models_pred_time = []
...
for model_path in model_paths:
    ...
    pred_time = []
    for _ in range(5):
        acsf_hp = acsf_hyperparameters[model_path.split("/")[-1]]
    ...
    print("For model %s, representation time: %.2f pm %.2f, prediction time: %.2f pm %.2f" % (model_path.split("/")[-1], np.mean(rep_time), np.std(rep_time), np.mean(pred_time), np.std(pred_time)))
