...
# Creating the model
estimator = MRMP(iterations=2401, l1_reg=3.7260078742338124e-06, l2_reg=2.1321843097427243e-07, learning_rate=0.0004, representation_name='slatm', tensorboard=False, hidden_layer_sizes=(32,298,), batch_size=15)

estimator.set_properties(ene)
estimator.generate_representation(xyz, zs, method='fortran')

estimator.fit(idx_train)
estimator.save_nn("../outputs/ch4cn_slatm_001/")

ene_pred = estimator.predict(idx_test)
ene_pred_traj = estimator.predict(idx_traj)

np.savez("../outputs/ch4cn_slatm_001.npz", ene_pred, ene[idx_test], ene_pred_traj, ene[idx_traj])
